...
#ifndef HOF_GUARD_IF_H
#define HOF_GUARD_IF_H
...
{
    HOF_INHERIT_CONSTRUCTOR(if_adaptor, detail::callable_base<F>)
};
...
struct make_if_f
{
    ...
    {}
    template<class F>
    constexpr if_adaptor<Cond, F> operator()(F f) const HOF_NOEXCEPT_CONSTRUCTIBLE(F, F&&)
    {
...
template<bool B>
HOF_STATIC_CONSTEXPR detail::make_if_f<B> if_c = {};
#else
template<bool B, class F>
constexpr detail::if_adaptor<B, F> if_c(F f) HOF_NOEXCEPT_CONSTRUCTIBLE(F, F&&)
{
    return detail::if_adaptor<B, F>(static_cast<F&&>(f));
}
#endif

HOF_DECLARE_STATIC_VAR(if_, detail::if_f);
