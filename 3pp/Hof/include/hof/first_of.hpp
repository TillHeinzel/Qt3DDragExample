...
#ifndef HOF_GUARD_FUNCTION_CONDITIONAL_H
#define HOF_GUARD_FUNCTION_CONDITIONAL_H
...
{
    HOF_INHERIT_DEFAULT(basic_first_of_adaptor, F1, F2)

    template<class A, class B,
        HOF_ENABLE_IF_CONVERTIBLE(A, F1),
        HOF_ENABLE_IF_CONVERTIBLE(B, F2)>
    constexpr basic_first_of_adaptor(A&& f1, B&& f2)
    noexcept(HOF_IS_NOTHROW_CONSTRUCTIBLE(F1, A&&) && HOF_IS_NOTHROW_CONSTRUCTIBLE(F2, B&&))
    : F1(HOF_FORWARD(A)(f1)), F2(HOF_FORWARD(B)(f2))
    {}

    template<class X,
        class=typename std::enable_if<
        HOF_IS_CONVERTIBLE(X, F1) && 
        HOF_IS_DEFAULT_CONSTRUCTIBLE(F2)
    >::type>
    constexpr basic_first_of_adaptor(X&& x) 
    HOF_NOEXCEPT_CONSTRUCTIBLE(F1, X&&)
    : F1(HOF_FORWARD(X)(x))
    {} 
    ...
    HOF_RETURNS_CLASS(basic_first_of_adaptor);

    template<class... Ts, class F=typename select<Ts...>::type>
    constexpr HOF_SFINAE_RESULT(typename select<Ts...>::type, id_<Ts>...) 
    operator()(Ts && ... xs) const
    HOF_SFINAE_RETURNS
    (
        HOF_RETURNS_STATIC_CAST(const F&)(*HOF_CONST_THIS)(HOF_FORWARD(Ts)(xs)...)
    );
};
...
struct conditional_kernel : compressed_pair<F1, F2>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(conditional_kernel, base)
    ...
    HOF_RETURNS_CLASS(conditional_kernel);
    ...
    (
        boost::hof::detail::which(
            HOF_RETURNS_CONSTRUCT(PickFirst)(),
            HOF_MANGLE_CAST(const F1&)(HOF_CONST_THIS->first(xs...)),
            HOF_MANGLE_CAST(const F2&)(HOF_CONST_THIS->second(xs...))
        )
        (HOF_FORWARD(Ts)(xs)...)
    );
};
...
: detail::conditional_kernel<F, HOF_JOIN(first_of_adaptor, Fs...) >
{
    typedef first_of_adaptor fit_rewritable_tag;
    typedef HOF_JOIN(first_of_adaptor, Fs...) kernel_base;
    typedef detail::conditional_kernel<F, kernel_base > base;

    HOF_INHERIT_DEFAULT(first_of_adaptor, base)

    template<class X, class... Xs, 
        HOF_ENABLE_IF_CONSTRUCTIBLE(base, X, kernel_base), 
        HOF_ENABLE_IF_CONSTRUCTIBLE(kernel_base, Xs...)>
    constexpr first_of_adaptor(X&& f1, Xs&& ... fs) 
    noexcept(HOF_IS_NOTHROW_CONSTRUCTIBLE(base, X&&, kernel_base) && HOF_IS_NOTHROW_CONSTRUCTIBLE(kernel_base, Xs&&...))
    : base(HOF_FORWARD(X)(f1), kernel_base(HOF_FORWARD(Xs)(fs)...))
    {}

    template<class X, class... Xs, 
        HOF_ENABLE_IF_CONSTRUCTIBLE(base, X)>
    constexpr first_of_adaptor(X&& f1) 
    HOF_NOEXCEPT_CONSTRUCTIBLE(base, X&&)
    : base(HOF_FORWARD(X)(f1))
    {}
...
{
    typedef first_of_adaptor fit_rewritable_tag;
    HOF_INHERIT_CONSTRUCTOR(first_of_adaptor, F);
...
: detail::conditional_kernel<F1, F2>
{
    ...
    typedef first_of_adaptor fit_rewritable_tag;
    HOF_INHERIT_CONSTRUCTOR(first_of_adaptor, base);
...
HOF_DECLARE_STATIC_VAR(first_of, detail::make<first_of_adaptor>);
