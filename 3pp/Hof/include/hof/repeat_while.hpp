...
#ifndef HOF_GUARD_REPEAT_WHILE_H
#define HOF_GUARD_REPEAT_WHILE_H
...
struct while_repeater
{
    ...
    constexpr HOF_SFINAE_RESULT(while_repeater<
        compute_predicate<P, typename result_of<const F&, id_<Ts>...>::type>::type::value
    >, id_<const F&>, id_<const P&>, result_of<const F&, id_<Ts>...>) 
    operator()(const F& f, const P& p, Ts&&... xs) const HOF_SFINAE_RETURNS
    (
        while_repeater<
            compute_predicate<P, decltype(f(HOF_FORWARD(Ts)(xs)...))>::type::value
        >()(f, p, f(HOF_FORWARD(Ts)(xs)...))
    );
};
...
struct repeat_while_constant_decorator
{
    ...
    constexpr auto operator()(const P& p, const F& f, Ts&&... xs) const HOF_RETURNS
    (
        detail::while_repeater<
            detail::compute_predicate<P, decltype(std::declval<F>()(HOF_FORWARD(Ts)(xs)...))>::type::value
        >()
        (
            f, 
            p, 
...
struct repeat_while_integral_decorator
{
    ...
    constexpr auto operator()(const P& p, const F& f, T&& x, Ts&&... xs) const HOF_RETURNS
    (
        (p(x, HOF_FORWARD(Ts)(xs)...)) ? 
            Self()(
                p, 
                f, 
                f(x, HOF_FORWARD(Ts)(xs)...)
            ) : 
            HOF_FORWARD(T)(x)
    );
};
...
struct repeat_while_integral_decorator<0>
{
    ...
    HOF_RETURNS_DEDUCE_NOEXCEPT((p(x), f(x)))
    -> decltype(f(x))
    {
        while(p(x))
        {
...
#if HOF_HAS_RELAXED_CONSTEXPR
#define HOF_REPEAT_WHILE_CONSTEXPR_DEPTH 1
#else
#define HOF_REPEAT_WHILE_CONSTEXPR_DEPTH HOF_RECURSIVE_CONSTEXPR_DEPTH
#endif

HOF_DECLARE_STATIC_VAR(repeat_while, decorate_adaptor<
    boost::hof::first_of_adaptor<
        detail::repeat_while_constant_decorator,
        detail::repeat_while_integral_decorator<HOF_REPEAT_WHILE_CONSTEXPR_DEPTH>
    >
>);
