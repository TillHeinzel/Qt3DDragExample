...
#ifndef HOF_GUARD_FUNCTION_REVEAL_H
#define HOF_GUARD_FUNCTION_REVEAL_H
...
/// 
///     template<class R>
///     auto adl_begin(R&& r) HOF_RETURNS(begin(r));
///     }
/// 
///     HOF_STATIC_LAMBDA_FUNCTION(for_each_tuple) = [](const auto& sequence, auto f) HOF_RETURNS
///     (
...
/// 
///         template<class T, class U>
///         auto operator()(T x, U y) const HOF_RETURNS(x+y);
///     };
/// 
///     int main() {
///         assert(sum_f()(1, 2) == 3);
...
#include <hof/is_invocable.hpp>
#include <hof/identity.hpp>
...
#ifndef HOF_REVEAL_USE_TEMPLATE_ALIAS
#ifdef __clang__
#define HOF_REVEAL_USE_TEMPLATE_ALIAS 1
#else
#define HOF_REVEAL_USE_TEMPLATE_ALIAS 0
#endif
#endif
...
struct identity_failure
{
    ...
    template<class T>
    static T&& val();
#if HOF_REVEAL_USE_TEMPLATE_ALIAS
    template<template<class...> class Template, class... Ts>
    HOF_USING(defer, Template<Ts...>);
#else
    template<template<class...> class Template, class... Ts>
...
HOF_USING_TYPENAME(apply_failure, Failure::template of<Ts...>);
...
struct with_failures
{
    typedef HOF_JOIN(failures, Fs...) children;
};
...
struct failures 
{
    ...
    HOF_USING(transform, with_failures<detail::transform_failures<Failure, Transform>, detail::transform_failures<Failures, Transform>...>);

    template<class F, class FailureBase=HOF_JOIN(failures, Failures...)>
    struct overloads
...
{
    template<class Transform>
    HOF_USING(transform, with_failures<detail::transform_failures<Failure, Transform>>);

    template<class F>
    HOF_USING(overloads, detail::traverse_failure<F, Failure>);
};
...
: detail::traverse_failure<Base>, Base
{
    ...
    HOF_INHERIT_CONSTRUCTOR(reveal_adaptor, Base);
};
...
{
    typedef reveal_adaptor fit_rewritable1_tag;
    HOF_INHERIT_CONSTRUCTOR(reveal_adaptor, reveal_adaptor<F>);
};

HOF_DECLARE_STATIC_VAR(reveal, detail::make<reveal_adaptor>);
