...
#ifndef HOF_GUARD_FUNCTION_IMPLICIT_H
#define HOF_GUARD_FUNCTION_IMPLICIT_H
...
struct implicit
{
    ...
    struct invoker
    {
        ...
        constexpr invoker(Pack pp) HOF_NOEXCEPT_CONSTRUCTIBLE(Pack, Pack&&)
        : p(boost::hof::move(pp))
        {}

        template<class X, class=typename std::enable_if<detail::is_implicit_callable<F<X>, Pack, X>::value>::type>
        constexpr operator X() const HOF_NOEXCEPT(noexcept(p(F<X>())))
        {
            return p(F<X>());
        }
    ...
    {
        template<class Pack>
        constexpr invoker<Pack> operator()(Pack p) const HOF_NOEXCEPT(noexcept(invoker<Pack>(boost::hof::move(p))))
        {
    ...
    HOF_RETURNS
    (
        HOF_RETURNS_CONSTRUCT(make_invoker)()(boost::hof::pack_basic(HOF_FORWARD(Ts)(xs)...))
    );
};
