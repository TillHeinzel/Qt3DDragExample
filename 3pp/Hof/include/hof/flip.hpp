...
#ifndef HOF_GUARD_FLIP_H
#define HOF_GUARD_FLIP_H
...
struct flip_adaptor : detail::callable_base<F>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(flip_adaptor, detail::callable_base<F>);
    ...
    HOF_RETURNS_CLASS(flip_adaptor);

    template<class T, class U, class... Ts>
    constexpr HOF_SFINAE_RESULT(const detail::callable_base<F>&, id_<U>, id_<T>, id_<Ts>...) 
    operator()(T&& x, U&& y, Ts&&... xs) const HOF_SFINAE_RETURNS
    (
        (HOF_MANGLE_CAST(const detail::callable_base<F>&)(HOF_CONST_THIS->base_function(xs...)))
            (HOF_FORWARD(U)(y), HOF_FORWARD(T)(x), HOF_FORWARD(Ts)(xs)...)
    );
};

HOF_DECLARE_STATIC_VAR(flip, detail::make<flip_adaptor>);
