...
#ifndef HOF_GUARD_FUNCTION_LAMBDA_H
#define HOF_GUARD_FUNCTION_LAMBDA_H
...
/// -----------
/// 
/// The `HOF_STATIC_LAMBDA` macro allows initializing non-capturing lambdas at
...
///     #include <cassert>
/// 
///     const constexpr auto add_one = HOF_STATIC_LAMBDA(int x)
///     {
///         return x + 1;
///     };
/// 
...
///     }
/// 
/// HOF_STATIC_LAMBDA_FUNCTION
/// ==========================
/// 
/// Description
/// -----------
/// 
/// The `HOF_STATIC_LAMBDA_FUNCTION` macro allows initializing a global
...
///
/// By default, all functions defined with `HOF_STATIC_LAMBDA_FUNCTION` use
/// the `boost::hof::reveal` adaptor to improve error messages.
/// 
/// Note: due to compiler limitations, a global function declared with
/// `HOF_STATIC_LAMBDA_FUNCTION` is not guaranteed to have a unique 
...
///     #include <cassert>
/// 
///     HOF_STATIC_LAMBDA_FUNCTION(add_one) = [](int x)
///     {
///         return x + 1;
///     };
///     int main() {
...
#if !HOF_HAS_CONSTEXPR_LAMBDA || !HOF_HAS_INLINE_LAMBDAS

#include <type_traits>
#include <utility>
#include <hof/detail/result_of.hpp>
#include <hof/reveal.hpp>
#include <hof/detail/constexpr_deduce.hpp>
#include <hof/function.hpp>


#ifndef HOF_REWRITE_STATIC_LAMBDA
#ifdef _MSC_VER
#define HOF_REWRITE_STATIC_LAMBDA 1
#else
#define HOF_REWRITE_STATIC_LAMBDA 0
#endif
#endif
...
struct static_function_wrapper
{
    ...
    HOF_RETURNS_CLASS(static_function_wrapper);

    template<class... Ts>
    HOF_SFINAE_RESULT(const F&, id_<Ts>...) 
    operator()(Ts&&... xs) const HOF_SFINAE_RETURNS
    (
        HOF_RETURNS_REINTERPRET_CAST(const F&)(*HOF_CONST_THIS)(HOF_FORWARD(Ts)(xs)...)
    );
};
...
#if HOF_HAS_CONSTEXPR_LAMBDA
#define HOF_STATIC_LAMBDA []
#else
#define HOF_DETAIL_MAKE_STATIC HOF_DETAIL_CONSTEXPR_DEDUCE boost::hof::detail::static_function_wrapper_factor()
#define HOF_STATIC_LAMBDA HOF_DETAIL_MAKE_STATIC = []
#endif

#if HOF_HAS_INLINE_LAMBDAS
#define HOF_STATIC_LAMBDA_FUNCTION HOF_STATIC_FUNCTION
#else
#define HOF_DETAIL_MAKE_REVEAL_STATIC(T) HOF_DETAIL_CONSTEXPR_DEDUCE_UNIQUE(T) boost::hof::detail::reveal_static_lambda_function_wrapper_factor<T>()
#define HOF_STATIC_LAMBDA_FUNCTION(name) \
struct fit_private_static_function_ ## name {}; \
HOF_STATIC_AUTO_REF name = HOF_DETAIL_MAKE_REVEAL_STATIC(fit_private_static_function_ ## name)
#endif
