...
#ifndef HOF_GUARD_FUNCTION_OVERLOAD_H
#define HOF_GUARD_FUNCTION_OVERLOAD_H
...
/// is different than the [`first_of`](/include/hof/conditional) adaptor which resolves
/// them based on order.
/// 
/// Synopsis
/// --------
...
struct match_adaptor<F, Fs...> : detail::callable_base<F>, match_adaptor<Fs...>
{
    ...
    HOF_INHERIT_DEFAULT(match_adaptor, detail::callable_base<F>, base);

    template<class X, class... Xs, HOF_ENABLE_IF_CONVERTIBLE(X, detail::callable_base<F>), HOF_ENABLE_IF_CONSTRUCTIBLE(base, Xs...)>
    constexpr match_adaptor(X&& f1, Xs&& ... fs) 
    : detail::callable_base<F>(HOF_FORWARD(X)(f1)), base(HOF_FORWARD(Xs)(fs)...)
    {}
...
struct match_adaptor<F> : detail::callable_base<F>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(match_adaptor, detail::callable_base<F>);
};

HOF_DECLARE_STATIC_VAR(match, detail::make<match_adaptor>);
