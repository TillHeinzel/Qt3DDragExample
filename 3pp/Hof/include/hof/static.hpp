...
#ifndef HOF_GUARD_FUNCTION_STATIC_H
#define HOF_GUARD_FUNCTION_STATIC_H
...
struct static_
{
    ...
    HOF_NOEXCEPT_CONSTRUCTIBLE(F)
    {
        static F f;
        return f;
    }

    HOF_RETURNS_CLASS(static_);

    template<class... Ts>
    HOF_SFINAE_RESULT(F, id_<Ts>...) 
    operator()(Ts && ... xs) const
    HOF_SFINAE_RETURNS(HOF_CONST_THIS->base_function()(HOF_FORWARD(Ts)(xs)...));
};
