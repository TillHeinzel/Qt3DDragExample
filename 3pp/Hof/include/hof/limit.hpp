...
#ifndef HOF_GUARD_LIMIT_H
#define HOF_GUARD_LIMIT_H
...
///         }
///     };
///     HOF_STATIC_FUNCTION(sum) = limit_c<2>(sum_f());
/// 
///     int main() {
///         assert(3 == sum(1, 2));
///     }
...
#include <hof/function_param_limit.hpp>
...
struct limit_adaptor : detail::callable_base<F>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(limit_adaptor, detail::callable_base<F>)
    ...
    HOF_RETURNS_CLASS(limit_adaptor);

    template<class... Ts, class=typename std::enable_if<(sizeof...(Ts) <= N)>::type>
    constexpr HOF_SFINAE_RESULT(const detail::callable_base<F>&, id_<Ts>...) 
...
HOF_DECLARE_STATIC_VAR(limit, detail::limit_f);
