...
#ifndef HOF_GUARD_CONSTRUCT_H
#define HOF_GUARD_CONSTRUCT_H
...
struct construct_f
{
    ...
    T operator()(Ts&&... xs) const HOF_NOEXCEPT_CONSTRUCTIBLE(T, Ts&&...)
    {
        storage buffer{};
        new(&buffer) T(HOF_FORWARD(Ts)(xs)...);
        storage_holder h(&buffer);
        return boost::hof::move(h.data());
    }

    template<class X, HOF_ENABLE_IF_CONSTRUCTIBLE(T, std::initializer_list<X>&&)>
    T operator()(std::initializer_list<X>&& x) const HOF_NOEXCEPT_CONSTRUCTIBLE(T, std::initializer_list<X>&&)
    {
        storage buffer{};
    ...
    template<class X, HOF_ENABLE_IF_CONSTRUCTIBLE(T, const std::initializer_list<X>&)>
    T operator()(const std::initializer_list<X>& x) const HOF_NOEXCEPT_CONSTRUCTIBLE(T, const std::initializer_list<X>&)
    {
        storage buffer{};
        new(&buffer) T(x);
        storage_holder h(&buffer);
...
template<class T>
struct construct_f<T, typename std::enable_if<HOF_IS_LITERAL(T)>::type>
{
    constexpr construct_f() noexcept
    {}
    template<class... Ts, HOF_ENABLE_IF_CONSTRUCTIBLE(T, Ts...)>
...
struct construct_template_f
{
    ...
    {}
    template<class... Ts, class Result=HOF_JOIN(Template, typename D<Ts>::type...), 
        HOF_ENABLE_IF_CONSTRUCTIBLE(Result, Ts...)>
    constexpr Result operator()(Ts&&... xs) const HOF_NOEXCEPT_CONSTRUCTIBLE(Result, Ts&&...)
    {
        return construct_f<Result>()(HOF_FORWARD(Ts)(xs)...);
    }
};
...
struct construct_meta_f
{
    ...
    template<class... Ts, 
        class Metafunction=HOF_JOIN(apply, Ts...), 
...
struct construct_meta_template_f
{
    ...
    {}
    template<class... Ts, 
        class Metafunction=HOF_JOIN(MetafunctionTemplate, Ts...), 
        class Result=typename Metafunction::type, 
        HOF_ENABLE_IF_CONSTRUCTIBLE(Result, Ts...)>
    constexpr Result operator()(Ts&&... xs) const HOF_NOEXCEPT_CONSTRUCTIBLE(Result, Ts&&...)
    {
