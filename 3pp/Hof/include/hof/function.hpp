...
#ifndef HOF_GUARD_FUNCTION_FUNCTION_H
#define HOF_GUARD_FUNCTION_FUNCTION_H
...
/// The `HOF_STATIC_FUNCTION` macro allows initializing a function object from a
...
/// 
/// By default, all functions defined with `HOF_STATIC_FUNCTION` use the
/// [`boost::hof::reveal`](/include/hof/reveal) adaptor to improve error messages.
/// 
/// Example
/// -------
/// 
...
///     HOF_STATIC_FUNCTION(partial_sum) = boost::hof::partial(sum_f());
/// 
///     int main() {
...
#if HOF_HAS_INLINE_VARIABLES
#else
        static_assert(HOF_IS_DEFAULT_CONSTRUCTIBLE(F), "Static functions must be default constructible");
#endif
...
#if HOF_HAS_INLINE_VARIABLES
#define HOF_STATIC_FUNCTION(name) inline const constexpr auto name = boost::hof::detail::reveal_static_const_factory()
#else
#define HOF_STATIC_FUNCTION(name) HOF_STATIC_CONST_VAR(name) = HOF_DETAIL_MSVC_CONSTEXPR_DEDUCE boost::hof::detail::reveal_static_const_factory()
#endif
