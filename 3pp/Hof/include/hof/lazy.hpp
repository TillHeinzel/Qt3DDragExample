...
#ifndef HOF_GUARD_FUNCTION_LAZY_H
#define HOF_GUARD_FUNCTION_LAZY_H
...
struct ref_transformer
{
    ...
    HOF_SFINAE_RETURNS(boost::hof::always_ref(x.get()));
};
...
struct id_transformer
{
    ...
    HOF_SFINAE_RETURNS(always_detail::always_base<T>(HOF_FORWARD(T)(x)));
};

HOF_DECLARE_STATIC_VAR(pick_transformer, first_of_adaptor<placeholder_transformer, bind_transformer, ref_transformer, id_transformer>);

template<class T, class Pack>
constexpr auto lazy_transform(T&& x, const Pack& p) HOF_RETURNS
(
    p(boost::hof::detail::pick_transformer(HOF_FORWARD(T)(x)))
);
...
struct lazy_unpack
{
    ...
    (
        f(lazy_transform(HOF_FORWARD(Ts)(xs), p)...)
    );
};
...
: detail::compressed_pair<F, Pack>
{
...
#ifdef _MSC_VER
    HOF_INHERIT_CONSTRUCTOR(lazy_invoker, base_type)
#else
    HOF_INHERIT_DEFAULT_EMPTY(lazy_invoker, base_type)

    template<class X, class Y, 
        HOF_ENABLE_IF_CONSTRUCTIBLE(base_type, X&&, Y&&)
    >
    constexpr lazy_invoker(X&& x, Y&& y) 
    HOF_NOEXCEPT_CONSTRUCTIBLE(base_type, X&&, Y&&)
    : base_type(HOF_FORWARD(X)(x), HOF_FORWARD(Y)(y))
    {}
#endif
    ...
    HOF_RETURNS_CLASS(lazy_invoker);

    template<class... Ts>
    constexpr auto operator()(Ts&&... xs) const HOF_RETURNS
    (
        HOF_MANGLE_CAST(const Pack&)(HOF_CONST_THIS->get_pack(xs...))(
            boost::hof::detail::make_lazy_unpack(
                HOF_MANGLE_CAST(const F&)(HOF_CONST_THIS->base_function(xs...)), 
                boost::hof::pack_forward(HOF_FORWARD(Ts)(xs)...)
            )
        )
    );
};
...
HOF_NOEXCEPT_CONSTRUCTIBLE(lazy_invoker<F, Pack>, F&&, Pack&&)
{
...
{
    HOF_INHERIT_CONSTRUCTOR(lazy_nullary_invoker, F);
    ...
    HOF_RETURNS_CLASS(lazy_nullary_invoker);
...
HOF_NOEXCEPT_CONSTRUCTIBLE(lazy_nullary_invoker<F>, F&&)
{
...
{
    HOF_INHERIT_CONSTRUCTOR(lazy_adaptor, detail::callable_base<F>);
    ...
    HOF_RETURNS_CLASS(lazy_adaptor);

    template<class T, class... Ts>
    constexpr auto operator()(T x, Ts... xs) const HOF_RETURNS
    (
        boost::hof::detail::make_lazy_invoker(HOF_RETURNS_C_CAST(detail::callable_base<F>&&)(HOF_CONST_THIS->base_function(x, xs...)), 
            boost::hof::pack_basic(HOF_RETURNS_STATIC_CAST(T&&)(x), HOF_RETURNS_STATIC_CAST(Ts&&)(xs)...))
    );
    ...
    HOF_RETURNS_DEDUCE_NOEXCEPT(
        boost::hof::detail::make_lazy_nullary_invoker(HOF_RETURNS_C_CAST(detail::callable_base<F>&&)(
            HOF_CONST_THIS->base_function(HOF_RETURNS_CONSTRUCT(Unused)())
        ))
    )
    {
    ...
    // template<class... Ts>
    // constexpr auto operator()(Ts... xs) && HOF_RETURNS
    // (
...
HOF_DECLARE_STATIC_VAR(lazy, detail::make<lazy_adaptor>);
