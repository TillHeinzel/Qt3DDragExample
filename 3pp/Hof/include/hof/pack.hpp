...
#ifndef HOF_GUARD_FUNCTION_PACK_H
#define HOF_GUARD_FUNCTION_PACK_H
...
#include <hof/detail/remove_rvalue_reference.hpp>
...
: std::integral_constant<bool, (
    HOF_IS_CONSTRUCTIBLE(T, const T&)
)>
{};
...
, int>::type = 0>
constexpr T pack_get(X&& x, Ts&&... xs) noexcept(HOF_IS_NOTHROW_COPY_CONSTRUCTIBLE(T))
{
    return static_cast<T>(boost::hof::alias_value<Tag, T>(HOF_FORWARD(X)(x), xs...));
}
...
template<class T, class Tag, class X, class... Ts, typename std::enable_if<
    !is_copyable<T>::value
, int>::type = 0>
constexpr auto pack_get(X&& x, Ts&&... xs) HOF_RETURNS
(
    boost::hof::alias_value<Tag, T>(HOF_FORWARD(X)(x), xs...)
);
...
: Ts::type...
{
    ...
    HOF_NOEXCEPT(HOF_AND_UNPACK(HOF_IS_NOTHROW_CONSTRUCTIBLE(typename Ts::type, Xs&&)))
    : Ts::type(HOF_FORWARD(Xs)(xs))...
    {}
#ifndef _MSC_VER
    // HOF_INHERIT_DEFAULT(pack_holder_base, typename std::remove_cv<typename std::remove_reference<typename Ts::type>::type>::type...)
    HOF_INHERIT_DEFAULT(pack_holder_base, typename Ts::type...)
#endif
};
...
: T::type
{
    ...
    HOF_INHERIT_CONSTRUCTOR(pack_holder_base, base);
};
...
: pack_holder_base<typename pack_holder_builder<Ts...>::template apply<Ts, Ns>...>
{
    ...
    HOF_NOEXCEPT(HOF_IS_NOTHROW_CONSTRUCTIBLE(base, X1&&, X2&&, Xs&...))
    : base(HOF_FORWARD(X1)(x1), HOF_FORWARD(X2)(x2), HOF_FORWARD(Xs)(xs)...)
    {}

    template<class X1, typename std::enable_if<(HOF_IS_CONSTRUCTIBLE(base, X1)), int>::type = 0>
    constexpr pack_base(X1&& x1)
    HOF_NOEXCEPT(HOF_IS_NOTHROW_CONSTRUCTIBLE(base, X1&&))
    : base(HOF_FORWARD(X1)(x1))
    {}

    // HOF_INHERIT_DEFAULT(pack_base, typename std::remove_cv<typename std::remove_reference<Ts>::type>::type...);
    HOF_INHERIT_DEFAULT(pack_base, Ts...);

    HOF_RETURNS_CLASS(pack_base);
  
    template<class F>
    constexpr auto operator()(F&& f) const HOF_RETURNS
    (
        f(boost::hof::detail::pack_get<Ts, pack_tag<seq<Ns>, Ts...>>(*HOF_CONST_THIS, f)...)
    );
...
: pack_holder_base<pack_holder<T, pack_tag<seq<0>, T>>>
{
    ...
    HOF_INHERIT_DEFAULT(pack_base, T);
    ...
    constexpr auto operator()(F&& f) const HOF_RETURNS
    (
        f(boost::hof::detail::pack_get<T, pack_tag<seq<0>, T>>(*HOF_CONST_THIS, f))
    );
...
: pack_holder<Ts, pack_tag<seq<Ns>, Ts...>>::type...
{
    ...
    template<class... Xs, HOF_ENABLE_IF_CONVERTIBLE_UNPACK(Xs&&, typename pack_holder<Ts, pack_tag<seq<Ns>, Ts...>>::type)>
    constexpr pack_base(Xs&&... xs) 
    HOF_NOEXCEPT(HOF_AND_UNPACK(HOF_IS_NOTHROW_CONSTRUCTIBLE(typename pack_holder<Ts, pack_tag<seq<Ns>, Ts...>>::type, Xs&&)))
    : pack_holder<Ts, pack_tag<seq<Ns>, Ts...>>::type(HOF_FORWARD(Xs)(xs))...
    {}
...
#define HOF_DETAIL_UNPACK_PACK_BASE(ref, move) \
template<class F, std::size_t... Ns, class... Ts> \
constexpr auto unpack_pack_base(F&& f, pack_base<seq<Ns...>, Ts...> ref x) \
HOF_RETURNS(f(boost::hof::alias_value<pack_tag<seq<Ns>, Ts...>, Ts>(move(x), f)...))
HOF_UNARY_PERFECT_FOREACH(HOF_DETAIL_UNPACK_PACK_BASE)
...
struct pack_join_base<pack_base<seq<Ns1...>, Ts1...>, pack_base<seq<Ns2...>, Ts2...>>
{
    ...
    HOF_RETURNS_DEDUCE_NOEXCEPT(
        result_type(
            boost::hof::detail::pack_get<Ts1, pack_tag<seq<Ns1>, Ts1...>>(HOF_FORWARD(P1)(p1))..., 
            boost::hof::detail::pack_get<Ts2, pack_tag<seq<Ns2>, Ts2...>>(HOF_FORWARD(P2)(p2))...)
    )
    {
        return result_type(
            boost::hof::detail::pack_get<Ts1, pack_tag<seq<Ns1>, Ts1...>>(HOF_FORWARD(P1)(p1))..., 
...
struct pack_basic_f
{
    ...
    (
        pack_base<typename gens<sizeof...(Ts)>::type, typename remove_rvalue_reference<Ts>::type...>(HOF_FORWARD(Ts)(xs)...)
    );
};
...
struct pack_forward_f
{
    ...
    constexpr auto operator()(Ts&&... xs) const HOF_RETURNS
    (
        pack_base<typename gens<sizeof...(Ts)>::type, Ts&&...>(HOF_FORWARD(Ts)(xs)...)
    );
};
...
struct pack_f
{
    ...
    constexpr auto operator()(Ts&&... xs) const HOF_RETURNS
    (
        pack_basic_f()(boost::hof::decay(HOF_FORWARD(Ts)(xs))...)
    );
};

template<class P1, class P2>
constexpr typename pack_join_result<P1, P2>::result_type make_pack_join_dual(P1&& p1, P2&& p2)
HOF_RETURNS_DEDUCE_NOEXCEPT(pack_join_result<P1, P2>::call(HOF_FORWARD(P1)(p1), HOF_FORWARD(P2)(p2)))
{
    return pack_join_result<P1, P2>::call(HOF_FORWARD(P1)(p1), HOF_FORWARD(P2)(p2));
}
...
template<class P1>
constexpr P1 make_pack_join(P1&& p1) HOF_NOEXCEPT_CONSTRUCTIBLE(P1, P1&&)
{
    return HOF_FORWARD(P1)(p1);
}

template<class P1, class... Ps>
constexpr typename join_type<P1, Ps...>::type make_pack_join(P1&& p1, Ps&&... ps)
HOF_RETURNS_DEDUCE_NOEXCEPT(make_pack_join_dual(HOF_FORWARD(P1)(p1), make_pack_join(HOF_FORWARD(Ps)(ps)...)))
{
    return make_pack_join_dual(HOF_FORWARD(P1)(p1), make_pack_join(HOF_FORWARD(Ps)(ps)...));
}
...
struct pack_join_f
{
    ...
    template<class... Ps>
    constexpr auto operator()(Ps&&... ps) const HOF_RETURNS
    (
        make_pack_join(HOF_FORWARD(Ps)(ps)...)
    );
};

}

HOF_DECLARE_STATIC_VAR(pack_basic, detail::pack_basic_f);
HOF_DECLARE_STATIC_VAR(pack_forward, detail::pack_forward_f);
HOF_DECLARE_STATIC_VAR(pack, detail::pack_f);

HOF_DECLARE_STATIC_VAR(pack_join, detail::pack_join_f);

template<class T, class... Ts>
struct unpack_sequence<detail::pack_base<T, Ts...>>
{
    template<class F, class P>
    constexpr static auto apply(F&& f, P&& p) HOF_RETURNS
    (
        boost::hof::detail::unpack_pack_base(HOF_FORWARD(F)(f), HOF_FORWARD(P)(p))
    );
};
