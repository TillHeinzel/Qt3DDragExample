...
#ifndef HOF_GUARD_FUNCTION_PARTIAL_H
#define HOF_GUARD_FUNCTION_PARTIAL_H
...
HOF_DECLARE_STATIC_VAR(partial, detail::make<partial_adaptor>);
...
struct partial_adaptor_invoke
{
    ...
    HOF_RETURNS_CLASS(partial_adaptor_invoke);

    template<class... Ts>
    constexpr HOF_SFINAE_RESULT
    (
    ...
    (
        ...
        (HOF_RETURNS_C_CAST(F&&)(HOF_CONST_THIS->get_function(xs...)))
    );
};

#ifdef _MSC_VER
#define HOF_PARTIAL_RETURNS(...) -> decltype(__VA_ARGS__) { return (__VA_ARGS__); }
#else
#define HOF_PARTIAL_RETURNS HOF_SFINAE_RETURNS
#endif
...
struct partial_adaptor_join
{
    ...
    HOF_RETURNS_CLASS(partial_adaptor_join);
    ...
    noexcept(HOF_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && noexcept(boost::hof::pack_join(std::declval<const Pack&>(), boost::hof::pack(HOF_FORWARD(Ts)(xs)...))))
#endif
    HOF_PARTIAL_RETURNS
    (
        boost::hof::partial
        (
            HOF_RETURNS_C_CAST(F&&)(HOF_CONST_THIS->get_function(xs...)), 
            boost::hof::pack_join(HOF_MANGLE_CAST(const Pack&)(HOF_CONST_THIS->get_pack(xs...)), boost::hof::pack(HOF_FORWARD(Ts)(xs)...))
        )
    );
};
...
struct partial_adaptor_pack
{
    ...
    HOF_RETURNS_CLASS(partial_adaptor_pack);
...
#ifdef _MSC_VER
    // Workaround ICE on MSVC
    noexcept(HOF_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && noexcept(boost::hof::pack(HOF_FORWARD(Ts)(xs)...)))
#endif
    HOF_PARTIAL_RETURNS
    (
        boost::hof::partial
        (
            HOF_RETURNS_C_CAST(F&&)(HOF_CONST_THIS->get_function(xs...)), 
            boost::hof::pack(HOF_FORWARD(Ts)(xs)...)
        )
    );
};
...
struct partial_adaptor : detail::partial_adaptor_base<F, Pack>::type, F, Pack
{
    ...
    HOF_INHERIT_DEFAULT(partial_adaptor, base, F, Pack);

    template<class X, class S>
    constexpr partial_adaptor(X&& x, S&& seq) 
    HOF_NOEXCEPT(HOF_IS_NOTHROW_CONSTRUCTIBLE(F, X&&) && HOF_IS_NOTHROW_CONSTRUCTIBLE(Pack, S&&))
    : F(HOF_FORWARD(X)(x)), Pack(HOF_FORWARD(S)(seq))
    {}
};
...
struct partial_adaptor<F, void> : detail::partial_adaptor_pack_base<partial_adaptor<F, void>, detail::callable_base<F>>::type
{
    ...
    HOF_INHERIT_CONSTRUCTOR(partial_adaptor, base);
