...
#ifndef HOF_GUARD_IS_UNPACKABLE_HPP
#define HOF_GUARD_IS_UNPACKABLE_HPP
...
#include <hof/detail/unpack_tuple.hpp>
...
struct unpack_impl_f
{
    ...
    constexpr auto operator()(F&& f, Sequence&& s) const HOF_RETURNS
    (
        boost::hof::unpack_sequence<typename std::remove_cv<typename std::remove_reference<Sequence>::type>::type>::
                apply(HOF_FORWARD(F)(f), HOF_FORWARD(Sequence)(s))
    );
};

HOF_DECLARE_STATIC_VAR(unpack_impl, unpack_impl_f);

#if HOF_CHECK_UNPACK_SEQUENCE
