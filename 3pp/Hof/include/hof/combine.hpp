...
#ifndef HOF_GUARD_COMBINE_H
#define HOF_GUARD_COMBINE_H
...
: F, pack_base<seq<Ns...>, Gs...>
{
    ...
    HOF_INHERIT_DEFAULT(combine_adaptor_base, base_type, F)

    template<class X, class... Xs, 
        HOF_ENABLE_IF_CONSTRUCTIBLE(F, X),
        HOF_ENABLE_IF_CONSTRUCTIBLE(base_type, Xs...)>
    constexpr combine_adaptor_base(X&& x, Xs&&... xs) 
    : F(HOF_FORWARD(X)(x)), base_type(HOF_FORWARD(Xs)(xs)...)
    {}
    ...
    HOF_RETURNS_CLASS(combine_adaptor_base);

// Result needs to be calculated in a separate class to avoid confusing the
// compiler on MSVC
#if HOF_NO_EXPRESSION_SFINAE || HOF_HAS_MANUAL_DEDUCTION
    template<class... Ts>
    ...
    constexpr auto
#endif
    operator()(Ts&&... xs) const HOF_SFINAE_MANUAL_RETURNS
    (
        (HOF_MANGLE_CAST(const F&)(HOF_CONST_THIS->base_function(xs...)))
            (boost::hof::alias_value<pack_tag<seq<Ns>, Gs...>, Gs>(*HOF_CONST_THIS, xs)(HOF_FORWARD(Ts)(xs))...)
    );
};
...
: detail::combine_adaptor_base<typename detail::gens<sizeof...(Gs)>::type, detail::callable_base<F>, detail::callable_base<Gs>...>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(combine_adaptor, base_type)
};

HOF_DECLARE_STATIC_VAR(combine, detail::make<combine_adaptor>);
