...
#ifndef HOF_GUARD_CAPTURE_H
#define HOF_GUARD_CAPTURE_H
...
struct capture_invoke : detail::compressed_pair<detail::callable_base<F>, Pack>, detail::function_result_type<F>
{
    ...
    HOF_INHERIT_CONSTRUCTOR(capture_invoke, base)
    template<class... Ts>
    ...
    HOF_RETURNS_CLASS(capture_invoke);
    ...
    (
        ...
        (HOF_RETURNS_C_CAST(detail::callable_base<F>&&)(HOF_CONST_THIS->base_function(xs...)))
    );
};
...
{
    HOF_INHERIT_CONSTRUCTOR(capture_pack, Pack);

    HOF_RETURNS_CLASS(capture_pack);

    // TODO: Should use rvalue ref qualifier
    template<class F>
    constexpr auto operator()(F f) const HOF_SFINAE_RETURNS
    (
        capture_invoke<F, Pack>(HOF_RETURNS_STATIC_CAST(F&&)(f), 
            HOF_RETURNS_C_CAST(Pack&&)(
                HOF_RETURNS_STATIC_CAST(const Pack&)(*boost::hof::always(HOF_CONST_THIS)(f))
            )
        )
    );
};
...
struct make_capture_pack_f
{
    ...
    HOF_NOEXCEPT_CONSTRUCTIBLE(capture_pack<Pack>, Pack&&)
    {
...
struct capture_f
{
    ...
    (
        HOF_RETURNS_CONSTRUCT(make_capture_pack_f)()(HOF_RETURNS_CONSTRUCT(F)()(HOF_FORWARD(Ts)(xs)...))
    );
};
}

HOF_DECLARE_STATIC_VAR(capture_basic, detail::capture_f<detail::pack_basic_f>);
HOF_DECLARE_STATIC_VAR(capture_forward, detail::capture_f<detail::pack_forward_f>);
HOF_DECLARE_STATIC_VAR(capture, detail::capture_f<detail::pack_f>);
